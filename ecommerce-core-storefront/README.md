...
Production hosting must route unknown storefront paths (such as `/products/example`) to `index.html`. Set the API `STOREFRONT_URL` to the public storefront origin so canonical sitemap URLs are correct. The dynamic sitemap is exposed at `/api/seo/sitemap.xml` and `public/robots.txt` references it.
