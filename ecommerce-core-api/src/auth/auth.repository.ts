...
interface UserRecord{id:string;email:string;full_name:string;password_hash:string;is_active:boolean;role:string;permissions:string[]}
...
export class AuthRepository{
  ...
  async findSession(id:string):Promise<SessionRecord|null>{
    const result=await this.database.query<SessionRecord>('SELECT id,admin_user_id,refresh_token_hash,expires_at,revoked_at FROM admin_sessions WHERE id=$1',[id]);
    return result.rows[0]??null;
  }
  async rotateSession(id:string,hash:string,expiresAt:Date,ip:string|null,agent:string|null):Promise<void>{
    await this.database.query(`UPDATE admin_sessions SET refresh_token_hash=$2,expires_at=$3,ip_address=$4,user_agent=$5,
