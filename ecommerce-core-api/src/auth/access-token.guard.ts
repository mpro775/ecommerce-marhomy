...
interface UserRow{id:string;email:string;full_name:string;role:string;permissions:string[];session_id:string}
@Injectable()
export class AccessTokenGuard implements CanActivate{
  ...
  async canActivate(context:ExecutionContext):Promise<boolean>{
    ...
    const user:AuthUser={id:row.id,email:row.email,fullName:row.full_name,role:row.role,
