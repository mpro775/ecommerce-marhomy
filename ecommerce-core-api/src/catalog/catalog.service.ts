...
import type { CreateCatalogValueDto, UpsertCatalogEntryDto } from './dto';
...
export class CatalogService{
  ...
  async list(kind:Kind):Promise<unknown[]>{this.assertKind(kind);return (await this.database.query(
    'SELECT * FROM '+kind+' ORDER BY sort_order,created_at DESC')).rows;}
  async create(kind:Kind,input:UpsertCatalogEntryDto):Promise<unknown>{
    ...
    if(kind==='categories'){
      ...
      return (await this.database.query(`INSERT INTO categories(parent_id,title_ar,title_en,slug,description_ar,description_en,image_url,is_active,sort_order)
        VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,[input.parentId??null,input.titleAr,input.titleEn??null,input.slug,
        input.descriptionAr??null,input.descriptionEn??null,input.imageUrl??null,input.isActive??true,input.sortOrder??0])).rows[0];
    }
    if(kind==='brands'){
      if(!input.titleAr)throw new BadRequestException('Arabic title is required');
      return (await this.database.query(`INSERT INTO brands(title_ar,title_en,slug,description_ar,description_en,logo_url,is_active,sort_order)
        VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,[input.titleAr,input.titleEn??null,input.slug,input.descriptionAr??null,
        input.descriptionEn??null,input.logoUrl??null,input.isActive??true,input.sortOrder??0])).rows[0];
    }
    if(!input.nameAr)throw new BadRequestException('Arabic name is required');
    if(kind==='attributes')return (await this.database.query(`INSERT INTO attributes(name_ar,name_en,slug,input_type,is_filterable,sort_order)
      VALUES($1,$2,$3,$4,$5,$6) RETURNING *`,[input.nameAr,input.nameEn??null,input.slug,input.inputType??'select',input.isFilterable??true,input.sortOrder??0])).rows[0];
  ...
  private allowedColumns(kind:Kind):Set<string>{
    const map:Record<Kind,string[]>={categories:['parent_id','title_ar','title_en','slug','description_ar','description_en','image_url','is_active','sort_order'],
      brands:['title_ar','title_en','slug','description_ar','description_en','logo_url','is_active','sort_order'],
      attributes:['name_ar','name_en','slug','input_type','is_filterable','sort_order'],
